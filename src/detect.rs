...
pub fn discover_projects(root: &Path) -> Result<Vec<ProjectConfig>> {
    ...
    walker.filter_entry(|entry| {
        ...
        !matches!(
            name.as_ref(),
            ".git" | "node_modules" | "target" | "provenance" | "dist"
        )
    });
...
mod tests {
    ...
    #[test]
    fn ignores_generated_provenance_directory() {
        let temp = tempdir().unwrap();
        let root = temp.path();

        fs::write(root.join("package.json"), r#"{"name":"root"}"#).unwrap();
        fs::create_dir_all(root.join("provenance/projects/child")).unwrap();
        fs::write(
            root.join("provenance/projects/child/package.json"),
        ...
        assert_eq!(projects[0].id, "root");
        assert_eq!(projects[0].path, PathBuf::from("."));
        assert_eq!(projects[0].ecosystems, vec![Ecosystem::Javascript]);
    }
}
