...
fn write_minimal_cargo_lock(root: &Path, package: &str, version: &str) {
    write_file(
...
fn generate_creates_outputs_for_projects_and_containers() {
    ...
    write_minimal_cargo_lock(temp.path(), "root", "0.1.0");
    write_file(
        &temp.path().join(".provenance.yml"),
        r"version: 1
output_dir: provenance
...
",
    ...
    assert!(repo_notice.contains("inventories of detected third-party components"));

    let project_notice =
        fs::read_to_string(temp.path().join("provenance/THIRD-PARTY-NOTICES.txt")).unwrap();
    assert!(project_notice.contains("Project: root"));
    assert!(project_notice.contains("@scope/alpha 1.2.3"));

    assert!(temp.path().join("provenance/sbom.cdx.json").exists());
    ...
    assert!(report.contains("\"sbom\": \"sbom.cdx.json\""));
    assert!(report.contains("\"notice\": \"THIRD-PARTY-NOTICES.txt\""));
    assert!(report.contains("\"name\": \"stella-core\""));
}

#[test]
fn generate_keeps_project_subdirectories_for_multi_project_repos() {
    let temp = TempDir::new().unwrap();
    let stubs = TempDir::new().unwrap();
    let cdxgen = write_cdxgen_stub(stubs.path());
    ...
    write_file(&temp.path().join("package.json"), r#"{"name":"root"}"#);
    write_file(
        &temp.path().join("packages/cli/package.json"),
        r#"{"name":"cli"}"#,
    );
    write_file(
        &temp.path().join(".provenance.yml"),
...
",
    ...
    assert!(
        temp.path()
            .join("provenance/projects/root/sbom.cdx.json")
            .exists()
    );
    assert!(
        temp.path()
            .join("provenance/projects/cli/sbom.cdx.json")
            .exists()
    );
    assert!(
        temp.path()
            .join("provenance/THIRD-PARTY-NOTICES.repo.txt")
            .exists()
    );
    assert!(!temp.path().join("provenance/sbom.cdx.json").exists());
    assert!(
        !temp
            .path()
            .join("provenance/THIRD-PARTY-NOTICES.txt")
            .exists()
    );
}

#[test]
fn generate_omits_repo_notice_for_single_project_without_containers() {
    let temp = TempDir::new().unwrap();
    let stubs = TempDir::new().unwrap();
    let cdxgen = write_cdxgen_stub(stubs.path());
...
",
    ...
    write_file(
        &temp.path().join("provenance/THIRD-PARTY-NOTICES.txt"),
        "tampered\n",
    );
    ...
    cargo_bin()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("=== THIRD-PARTY-NOTICES.txt ==="))
        .stdout(predicate::str::contains("--- checked-in"))
        .stdout(predicate::str::contains("+++ generated"));
}

#[test]
fn generate_refuses_to_write_into_repo_root() {
    let temp = TempDir::new().unwrap();
    let stubs = TempDir::new().unwrap();
    let cdxgen = write_cdxgen_stub(stubs.path());
...
",
    ...
    cargo_bin()
        ...
        .stderr(predicate::str::contains(
            "refusing to write provenance outputs into the repository root",
        ));
}
