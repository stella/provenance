...
fn init_discovers_workspace_roots() {
    ...
    cargo_bin()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains(".provenance.yml"));

    let config = fs::read_to_string(temp.path().join(".provenance.yml")).unwrap();
    assert!(config.contains("id: root"));
    assert!(config.contains("path: ."));
    assert!(config.contains("- javascript"));
    assert!(config.contains("- rust"));
    assert!(config.contains("output_dir: provenance"));
...
",
    ...
    assert!(repo_notice.contains("Generated by provenance."));

    let report = fs::read_to_string(temp.path().join("provenance/report.json")).unwrap();
...
",
    ...
    write_file(
        &temp
            .path()
            .join("provenance/projects/root/THIRD-PARTY-NOTICES.txt"),
        "tampered\n",
    );
    ...
    cargo_bin()
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Generated outputs are stale"))
