...
pub fn run(root: PathBuf, config_path: Option<PathBuf>, output_dir: Option<PathBuf>) -> Result<()> {
    ...
    ensure_safe_output_dir(&root, &output_path)?;
    let report_output_dir = display_relative(&root, &output_path);
    let report = generate_all(&root, &config, &output_path, &report_output_dir)?;
...
pub fn generate_all(
    ...
    let mut report = ComplianceReport {
        version: 1,
        output_dir: report_output_dir.to_string(),
        projects: Vec::new(),
        containers: Vec::new(),
    };
    ...
    for project in &config.projects {
        ...
        let sbom = generate_project_sbom(
            root,
            project,
            &sbom_path,
...
pub(crate) fn ensure_safe_output_dir(root: &Path, output_path: &Path) -> Result<()> {
    if normalize_path(root) == normalize_path(output_path) {
        return Err(miette!(
            "refusing to write provenance outputs into the repository root; set output_dir to a dedicated directory such as `provenance`"
        ));
    }
